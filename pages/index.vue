        ...
        <div class="flex w-full flex-col lg:flex-row">
          ...
          <div class="flex-grow">
            <table class="w-full">
              ...
              <tbody>
                <template v-if="logs.length > 0">
                  <tr v-for="log in logs" :key="log.date.toISOString()">
...
function updateHeight() {
  requestAnimationFrame(() => {
    detailHeight.value = detailsRef.value?.open
      ? detailsRef.value.getBoundingClientRect().height
      : 0;
  });
}
...
onMounted(() => {
  ...
  if (detailsRef.value) {
    const detailsElement = detailsRef.value as unknown as Element;
...
onBeforeUnmount(() => {
  if (resizeObserver && detailsRef.value) {
    ...
    resizeObserver.unobserve(detailsElement);
  }
});
</script>
